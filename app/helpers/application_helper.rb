module ApplicationHelper
  ...
  def motif_info(collection_name, motif)
    if    [:hocomoco_14_core, :hocomoco_14_rsnp, :hocomoco_14_rsnp_hq , :hocomoco_14_invivo, :hocomoco_14_invitro].include?(collection_name)
      motif_url = "https://hocomoco14.autosome.org/motif/#{motif}"
    elsif    [:hocomoco_13_core, :hocomoco_13_rsnp, :hocomoco_13_rsnp_hq , :hocomoco_13_invivo, :hocomoco_13_invitro].include?(collection_name)
      motif_url = "https://hocomoco13.autosome.org/motif/#{motif}"
    elsif [:hocomoco_12_core, :hocomoco_12_rsnp, :hocomoco_12_rsnp_hq , :hocomoco_12_invivo, :hocomoco_12_invitro].include?(collection_name)
  ...
  def annotation_by_h12_collection(collection_name)
    ...
    @cache_annotation_by_collection[collection_name] ||= begin
      mapping = {'hocomoco_12_rsnp_hq' => 'hocomoco_12_rsnp', :hocomoco_12_rsnp_hq => :hocomoco_12_rsnp}
      collection_name = mapping.fetch(collection_name, collection_name)
  ...
  def annotation_by_h13_collection(collection_name)
    @cache_annotation_by_collection ||= {}
    @cache_annotation_by_collection[collection_name] ||= begin
      mapping = {'hocomoco_13_rsnp_hq' => 'hocomoco_13_rsnp', :hocomoco_13_rsnp_hq => :hocomoco_13_rsnp}
      collection_name = mapping.fetch(collection_name, collection_name)
      collection_suffix = collection_name.to_s.split('_').last.upcase
      annotation_fn = Rails.root.join("public/motif_collection/H13#{collection_suffix}_annotation.jsonl")
      File.readlines(annotation_fn).map{|l|
        JSON.parse(l)
      }.group_by{|d|
        d['name']
  ...
  def annotation_by_h14_collection(collection_name)
    @cache_annotation_by_collection ||= {}
    @cache_annotation_by_collection[collection_name] ||= begin
      mapping = {'hocomoco_14_rsnp_hq' => 'hocomoco_14_rsnp', :hocomoco_14_rsnp_hq => :hocomoco_14_rsnp}
      collection_name = mapping.fetch(collection_name, collection_name)
      collection_suffix = collection_name.to_s.split('_').last.upcase
      annotation_fn = Rails.root.join("public/motif_collection/H14#{collection_suffix}_annotation.jsonl")
      File.readlines(annotation_fn).map{|l|
        JSON.parse(l)
      }.group_by{|d|
        d['name']
  ...
  def uniprot_links(collection_name, motif)
    if [:hocomoco_11_human, :hocomoco_11_mouse, :hocomoco_10_human, :hocomoco_10_mouse].include?(collection_name)
      ...
      uniprot_name = annotation_by_h13_collection(collection_name)[motif][:uniprot_id_human]
    elsif [:hocomoco_14_core, :hocomoco_14_rsnp, :hocomoco_14_rsnp_hq, :hocomoco_14_invivo, :hocomoco_14_invitro].include?(collection_name)
      uniprot_name = annotation_by_h14_collection(collection_name)[motif][:uniprot_id_human]
    end
